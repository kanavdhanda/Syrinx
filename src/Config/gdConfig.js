
const games = [
  {
    name: "level-1",
    args: [],
    canvasResizePolicy: 2,
    executable: "level-1",
    experimentalVK: true,
    fileSizes: { "level-1.pck": 24219360, "level-1": 1758196 },
    focusCanvas: true,
    gdextensionLibs: [],
    serviceWorker: "level-1.service.worker.js",
    mainPack:"https://srinx-ccs.blr1.cdn.digitaloceanspaces.com/level-1-pakka/level-1.pck",
  },
  // {
  //   name: "Level 2",
  //   args: [],
  //   canvasResizePolicy: 2,
  //   executable: "/level-2/level-2",
  //   experimentalVK: true,
  //   fileSizes: { "level-2.pck": 24219360, "level-2": 1758196 },
  //   focusCanvas: true,
  //   gdextensionLibs: [],
  //   serviceWorker: "level-2.service.worker.js",
  //   // mainPack:"https://srinx-ccs.blr1.cdn.digitaloceanspaces.com/level-1/level-1.pck",
  // },
];

export { games };


